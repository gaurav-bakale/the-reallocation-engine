# Tik TOC — A Practitioner's Guide

*Build a publication-ready Table of Contents from concept to adoptable textbook.*

Tik TOC is a senior instructional architect you run as a CLI-style command set. It thinks in backward design before chapter order, knows Bloom and Merrill by reflex, and won't let intellectual architecture override the publisher's adoption math. You drive it with slash commands; it gates the work into phases so a chapter never gets documented before the learning outcome it serves exists.

---

## Quick start

1. **Always begin with `/i1` (intake).** Tik TOC refuses to write a single chapter heading until the concept is coherent enough to structure.
...
|---|---|---|
| Intake | Asks before acting | Skips intake |
| Pushback | Flags weak input, names assumptions, reframes, disagrees | None |
| Phase gates | Enforced | None |
| Use when | Brief is thin, scope unclear, or the order may serve the author not the learner | You've done the thinking and want clean output to react to |

### The five phases

```
PHASE 1  Vision & Positioning      /i1 /i2 /i3 /i4
PHASE 2  Learning Architecture     /l1 /l2 /l3 /l4
PHASE 3  Chapter Architecture      /c1 /c2 /c3 /c4
PHASE 4  Scope & Market            /m1 /m2 /m3 /m4
PHASE 5  Production                /p1 /p2 /p3
BUILD    Finalization              /g1 /scaffold /g2 /g3 /g4 /g5
REFINE   Tools                     /logline /positioning /looptest /scopecheck
                                   /substack /volunteers /failmodes /changelog
```
...
> **Purpose:** Surface the foundational material before any TOC is written. Tik TOC asks one question at a time and refuses to proceed on incomplete answers.
